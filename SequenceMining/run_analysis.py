...
def get_raw_result(df,support,maxlen):
    ...
    #使用データの形に整形
    df_use = pd.DataFrame(sorted_df2, columns=["cookie_index","timestamp_index","url_index"])
    #リスト形式への変換
    data = df_use.apply(lambda row: [row['cookie_index'], row['timestamp_index'], [row['url_index']]], axis=1).tolist()
...
def get_user_pattern_table(df, sequences):
    ...
    for cid, group in df.groupby('cookie_index'):
        ...
        act_nums = group['url_index'].tolist()
...
def join_url(url_data,merged_df):
    # URLリストの辞書を作成
    url_dict = dict(zip(url_data['url_index'].astype(str), url_data.iloc[:, 0]))
